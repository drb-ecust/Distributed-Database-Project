...
@REM Test ClientDieRMAfterPrepare
...
@REM step 5: run ClientDieRMAfterPrepare
@REM wait some time to make sure rms, tm and wc have enough time to start up
timeout /t 5 /nobreak >nul
start "Client-Die-RM-After-Prepare" java -classpath %CLASS_PATH% -DrmiPort=%RMI_REGISTRY_PORT% cn.edu.fudan.ddb.client.ClientDieRMAfterPrepare

@REM step 6: recover RM Flight
@REM wait some time to make sure rm already die after prepare
timeout /t 5 /nobreak >nul
