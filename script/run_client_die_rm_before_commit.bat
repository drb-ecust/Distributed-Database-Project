...
@REM step 5: run ClientDieRMBeforeCommit
@REM wait some time to make sure rms, tm and wc have enough time to start up
timeout /t 5 /nobreak >nul
start "Client-Die-RM-Before-Commit" java -classpath %CLASS_PATH% -DrmiPort=%RMI_REGISTRY_PORT% cn.edu.fudan.ddb.client.ClientDieRMBeforeCommit

@REM step 6: recover TM
@REM wait some time to make sure rm already die before commit
timeout /t 5 /nobreak >nul
