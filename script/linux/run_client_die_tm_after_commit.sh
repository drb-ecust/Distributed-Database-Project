...
output_logs=("$LOG_DIR/output1.log" "$LOG_DIR/output2.log" "$LOG_DIR/output3.log" "$LOG_DIR/output4.log" "$LOG_DIR/output5.log" "$LOG_DIR/output6.log" "$LOG_DIR/output7.log" "$LOG_DIR/output8.log" "$LOG_DIR/output9.log")
error_logs=("$LOG_DIR/error1.log" "$LOG_DIR/error2.log" "$LOG_DIR/error3.log" "$LOG_DIR/error4.log" "$LOG_DIR/error5.log" "$LOG_DIR/error6.log" "$LOG_DIR/error7.log" "$LOG_DIR/error8.log" "$LOG_DIR/error9.log")
...
# step 5: run ClientDieTMAfterCommit
# wait some time to make sure rms, tm and wc have enough time to start up

java -classpath "$CLASS_PATH" -DrmiPort="$RMI_REGISTRY_PORT" database.client.ClientDieTMAfterCommit &

# step 6: recover TM
# wait some time to make sure rm already die after prepare
sleep 5
java -classpath "$CLASS_PATH" -DrmiPort="$RMI_REGISTRY_PORT" database.transaction.TransactionManagerImpl 1>"${output_logs[8]}" 2>"${error_logs[8]}" &
#java -classpath "$CLASS_PATH" -DrmiPort="$RMI_REGISTRY_PORT" database.transaction.TransactionManagerImpl &
