...
@REM Test ClientDieRMBeforePrepare
...
@REM step 5: run ClientDieRMBeforePrepare
@REM wait some time to make sure rms, tm and wc have enough time to start up
timeout /t 5 /nobreak >nul
start "Client-Die-RM-Before-Prepare" java -classpath %CLASS_PATH% -DrmiPort=%RMI_REGISTRY_PORT% cn.edu.fudan.ddb.client.ClientDieRMBeforePrepare

@REM step 6: recover RM Flight
@REM wait some time to make sure rm already die before prepare
timeout /t 5 /nobreak >nul
start "Flight-RM-Recovered" /min java -classpath %CLASS_PATH% -DrmiPort=%RMI_REGISTRY_PORT% cn.edu.fudan.ddb.resource.FlightResourceManager