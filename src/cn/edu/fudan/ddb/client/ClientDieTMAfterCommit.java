...
/**
 * Client code for test the case when TM die after commit
 */
public class ClientDieTMAfterCommit {
    public static void main(String[] args) {
        System.out.println("#################### Begin Test ClientDieTMAfterCommit ####################");
        ...
        try {
            ...
            //////////
            // new transaction 4: reserve Flight, Car and Room, but TM will die after commit
            //////////
            xid = wc.start();
            System.out.printf("### Start Transaction xid=%d: reserve Flight, Car and Room, but TM will die after commit.\n", xid);
            wc.dieTMAfterCommit(); // set a flag and info TM to die after the next commit
            ...
            if (!wc.commit(xid)) {
                System.err.printf("### Commit Transaction xid=%d: failed!\n", xid);
                System.exit(1);
            } else {
                System.out.printf("### Commit Transaction xid=%d: success! If TM die after commit, it will info each RM to recommit after recover, the consistency is still guaranteed.\n", xid);
            }


            //////////
            // new transaction 5: check consistency after processing TM die
            //////////
            xid = wc.start();
            System.out.printf("### Start Transaction xid=%d: check consistency after processing TM die.\n", xid);
            System.out.printf("Flight 1001 has %s available seats, the price is %d.\n",
                    wc.queryFlight(xid, "1001"), wc.queryFlightPrice(xid, "1001"));
            System.out.printf("Location ShangHai has %s available cars, the price is %d.\n",
                    wc.queryCars(xid, "ShangHai"), wc.queryCarsPrice(xid, "ShangHai"));
        ...
        System.out.println("#################### Finish Test ClientDieTMAfterCommit ####################");
